...
const PDFDocumentPreview: React.FC<PDFDocumentPreviewProps> = ({
            ...
            <div className="mr-2 flex-grow">
              ...
              {file.pageCount && (
                <div className="text-xs opacity-75">
                  {file.pageCount} {file.pageCount === 1 ? 'page' : 'pages'} • {file.size ? formatFileSize(file.size) : ''}
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className={`pdf-document-preview ${className}`}>
      {/* Controls header */}
      {pdfFiles.length > 0 && (
        <div className="flex justify-between items-center mb-4 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
      ...
      <div className={`${viewMode === 'grid' ? 'space-y-4' : 'space-y-4'}`}>
        {pdfFiles.length > 0 ? (
          pdfFiles.map(file => renderDocumentListItem(file))
        ) : (
          renderEmptyState()
        )}
      </div>
